...
# Citation: 
# PEREIRA, V. (2018). Project: Recommender Systems, File: Python-DM-Recommender Systems-CF-SVD.py, GitHub repository: <https://github.com/Valdecy/Recommender-Systems-CF-SVD>
...
def decomposition(Xdata, mean_centering = "global", k = 2, user_in_columns = True, graph = True):
    ...
    svd_U, svd_sigma, svd_V_transposed = linalg.svd(centered_list[0], full_matrices = False) 
...
df = pd.read_csv('Python-DM-Recommender Systems-CF-02-SVD.txt', sep = '\t')
X = df.iloc[:,1:]
X = X.set_index(df.iloc[:,0]) # First column as row names

# Calling the Function
svd = decomposition(X, mean_centering = "global", k = 2, user_in_columns = True, graph = True)
